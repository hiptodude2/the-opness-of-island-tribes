...
AtlasGui.DisplayOrder = 99
AtlasGui.IgnoreGuiInset = true
...
for _, Interface in ipairs(game:GetService("CoreGui"):GetChildren()) do
    if Interface.Name == AtlasGui.Name and Interface ~= AtlasGui then
        Interface:Destroy()
    end
end

function AtlasHubIsRunning()
	return AtlasGui.Parent == game:GetService('CoreGui')
end

local function AddConnection(Signal, Function)
	if (not AtlasHubIsRunning()) then
		return
	end
	local SignalConnect = Signal:Connect(Function)
	table.insert(AtlasConnections, SignalConnect)
	return SignalConnect
end

task.spawn(function()
	while (AtlasHubIsRunning()) do
		wait()
	end

	for _, Connection in next, AtlasConnections do
...
task.spawn(function()
    repeat task.wait() until game:GetService('Players').LocalPlayer.Character and game:GetService('Players').LocalPlayer.Character:FindFirstChild('Humanoid')
    if GameName == 'Island Tribes🌴' then
        if not getgenv().StoppingError then
            getgenv().StoppingError = true
            local message = require(game:GetService('Players').LocalPlayer:WaitForChild('PlayerScripts'):WaitForChild('Main'):WaitForChild("Message"))
            local stoperror
       	    stoperror = hookfunction(message.New, function(...)
        	    local arg2 = select(2, ...) 
                if arg2 == "Try again!" then
                    return
                end
                return stoperror(...)
            end)
...
do
    ...
    function utility:HandleGradientButton(element,callback)
        ...
        local con = AddConnection(UIS.InputEnded, function(input)
            if input.UserInputType==Enum.UserInputType.MouseButton1 then
    ...
    function utility:InitDragging(frame,button)
        ...
        local inputBegan =  AddConnection(button.MouseButton1Down, function()
            _dragging = true
            _dragging_offset = Vector2.new(mouse.X,mouse.Y)-frame.AbsolutePosition
        end)

        local inputEnded = AddConnection(mouse.Button1Up, function()
            _dragging = false
        ...
        local updateEvent
        LPH_JIT_MAX(function()
            updateEvent = AddConnection(Run.RenderStepped, function()
...
do
    function Library.new(info)
        ...
        local function makeLibrary()
            ...
            Converted["_UI_Library"].Parent = AtlasGui
            ...
            Converted["_Main"].Name = "Main"
            Converted["_Main"].Parent = AtlasGui
            ...
            Converted["_Notifications"].Parent = AtlasGui
            ...
            Converted["_Hint"].Name = "Hint"
            Converted["_Hint"].Parent = AtlasGui
            ...
            saveCoroutine = coroutine.create(function()
                LPH_JIT_MAX(function()
                    while utility:Wait() do
                        if (not AtlasHubIsRunning()) then return end
                        local edited_flags = {}
                        for i,v in pairs(flags) do
        ...
        if info.UseLoader then
            ...
            table.insert(_connections,utility:HandleGradientButton(main.Yes,function()
                ...
                if nextKey then
                    ...
                    AddConnection(check.Activated, doChecks)
            ...
            local function doClose()
                ...
                e:Play()
                AddConnection(e.Completed, function()
                    pcall(function()
                        loader:Destory()
                    end)
                end)
            ...
            AddConnection(closeBtn.Activated, (doClose))
        ...
        local sectionUpdateEvent do
            ...
            LPH_JIT_MAX(function()
                sectionUpdateEvent = AddConnection(Run.RenderStepped, function()
        ...
        local closePages do -- pages open and close
            ...
            LPH_JIT_MAX(function()
                pagesUpdate = AddConnection(Run.RenderStepped, function()
            ...
            AddConnection(openBtn.Activated, function()
                if state == false then
                    state = true
                    startTween(openSize)
                end
        ...
        do -- other buttons
            ...
            AddConnection(searchBtn.Activated, function()
            ...
            AddConnection(infoButton.Activated, function()
        ...
        do -- resize
            ...
            AddConnection(main:GetPropertyChangedSignal("Visible"), function()
            ...
            LPH_JIT_MAX(function()
                table.insert(_connections, AddConnection(Run.RenderStepped, function()
        ...
        do
            ...
            AddConnection(doSearchBtn.Activated, function()
                searchBar:ReleaseFocus()
            end)

            AddConnection(searchBar:GetPropertyChangedSignal("Text"), function()
        ...
        table.insert(self._connections, AddConnection(UIS.InputBegan, function(input,gpe)
    ...
    function Library:Notify(info)
        ...
        local function makeNotif()
            ...
            Converted["_Notification"].Size = UDim2.new(1, 0, 0, 100)
            ...
            Converted["_Body"].TextScaled = false
        ...
        local function tweenIn()
            ...
            repeat task.wait() until notif:FindFirstChildOfClass('Frame')
            notif:FindFirstChildOfClass('Frame').Position = start
        ...
        local function tweenOut()
            ...
            notif:FindFirstChildOfClass('Frame').Position = finish

            tween = TS:Create( notif:FindFirstChildOfClass('Frame'),TweenInfo.new(0.45,Enum.EasingStyle.Sine,Enum.EasingDirection.In,0,false,0),{
                ["Position"] = start;
            })
            AddConnection(tween.Completed, function()
                notif:Destroy()
        ...
        notif:FindFirstChildOfClass('Frame').No.Visible = info.ShowNoButton and true

        local yes = utility:CreateButtonObject( notif:FindFirstChildOfClass('Frame').Yes)
        local no = utility:CreateButtonObject( notif:FindFirstChildOfClass('Frame').No)

        AddConnection(yes.Activated, function()
            tweenOut()
            info.Callback(true)
        end)

        AddConnection(no.Activated, function()
            tweenOut()
            info.Callback(false)
        end)
        ...
        notif:FindFirstChildOfClass('Frame').Title.Text = info.Title
        notif:FindFirstChildOfClass('Frame').Body.Text = info.Content
...
do
    function Page.new(_self,info) -- used internally and has no argument checks, use Library::CreatePage instead
        ...
        AddConnection(selector.Button.Activated, function()
...
do
    function Element.CreateToggle(section,info)
        ...
        do -- toggling
            ...
            AddConnection(toggleBtn.Activated, function()
            ...
            bigButton.ZIndex = 0
            AddConnection(bigButton.Activated, function()
            ...
            local con
            LPH_JIT_MAX(function()
                con = AddConnection(Run.RenderStepped, function()
    ...
    function Element.CreateSlider(section,info)
        ...
        do -- sliding
            ...
            LPH_JIT_MAX(function()
                con = AddConnection(Run.RenderStepped, function(dt)
                    if not UIS:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) then
                        dragging = false
                    end
    ...
    function Element.CreateButton(section,info)
        ...
        do -- button
            ...
            local last = nil
            LPH_JIT_MAX(function()
                con = AddConnection(mouse.Move, function()
    ...
    function Element.CreateTextBox(section,info)
        ...
        do
            ...
            table.insert(_self._connections, AddConnection(UIS.InputBegan, function(input)
    ...
    function Element.CreateInteractable(section,info)
        ...
        local function createElement()
            ...
            Converted["_Interactable"].Name = "_Interactable"
        ...
        local element = createElement()
        local fr = element['_Interactable']:FindFirstChildOfClass('Frame')
        local color = _self.color
        ...
        LPH_JIT_MAX(function()
            table.insert(_self._connections, AddConnection(Run.RenderStepped, function()
                if fr and fr:FindFirstChild('Loading') and fr:FindFirstChild('Loading'):FindFirstChild('Loading') then
        ...
        do -- interactable
            ...
            AddConnection(btn.InputBegan, function(input)
                if input.UserInputType==Enum.UserInputType.MouseButton1 then
                    local m = 1.2
                    fr.BackgroundColor3 = Color3.new(color.R*m,color.G*m,color.B*m)
                end
            ...
            table.insert(_self._connections, AddConnection(UIS.InputEnded, function(input)
                if input.UserInputType==Enum.UserInputType.MouseButton1 then
                    fr.BackgroundColor3 = color
                end
            end))

            AddConnection(btn.InputEnded, function(input)
    ...
    function Element.CreateKeybind(section,info)
        ...
        do -- keybind
            ...
            AddConnection(fr.InputEnded, function(input)
    ...
    function Element.CreateDropdown(section,info)
        ...
        local isOpen,closeDropdown do -- dropdown
            ...
            local function toggleDropdown()
                ...
                if not open then
                    AddConnection(tween1.Completed, function(p)
            ...
            AddConnection(btn.Activated, (toggleDropdown))
        end
        ...
        local function makeButton(func)
            ...
            do -- button
                ...
                AddConnection(element.Destroying, function()
                    con:Disconnect()
                end)
                return element
            end
    ...
    function Element.CreateColorPicker(section,info)
        ...
        do -- dropdown
            ...
            table.insert(_self._connections, AddConnection(UIS.WindowFocusReleased, function()
                hovering = false
            end))
            LPH_JIT_MAX(function()
                table.insert(_self._connections, AddConnection(Run.RenderStepped, function()
        ...
        do -- color picking
            ...
            AddConnection(rainbowBtn.MouseButton1Down, function()
                d1 = true
                d2 = false
            end)

            AddConnection(secondBtn.MouseButton1Down, function()
                d2 = true
                d1 = false
            end)

            table.insert(_self._connections, AddConnection(UIS.InputEnded, function(inp)
